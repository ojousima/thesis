...
\subsubsection{Frequency domain results} \lebel{sect:emh_fd}
...
Output of piezoelectric harvester does not follow the excitation in a similar manner to electromagnetic harvester. The output voltage is clamped by diodes of rectification circuit, and the current waveform does not follow the voltage waveform. Power output waveforms have been presented in figure \ref{}. As before, the output can be read as 1 V \= 1 mW. 
...
A few ICs from different manufacturers were considered for the new circuit, namely Seiko S-882z \cite{SeikoInstruments2010} charge pumps, Linear Technology LTC3105 \cite{ltc3015} boost charger and Texas Instruments BQ255xx -series boost chargers. As Seiko ICs are poorly available and LTC3105 requires high start-up currents, BQ25504 \cite{bq25504} was chosen as the core for harvesting circuit. The schematic of harvesting circuit is presented in figure \ref{fig:bq25504}.

\begin{figure}[htb]
\begin{center}
\includegraphics[height=8cm]{images/own_dwg/circuit/bq25504.jpg}
\end{center}
\caption{\label{fig:bq25504} Schematic of revised harvester circuit}
\end{figure}

The revised harvesting circuit is able to start at 0.33 V input DC voltage, or at near 0.4 V RMS amplitude AC voltage contrasted to 5 V DC level of LTC3331-based circuit. Supercapacitor was chosen as energy storage for easy measurement of accumulated harvested energy. 
...
As the energy storage used in circuit is a supercapacitor and all subsequent measurements are based on values measured from capacitor, the supercapacitor was characterised in-circuit to obtain more accurate values for system performance. 
